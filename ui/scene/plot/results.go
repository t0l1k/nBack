...
func (r *ResultPlot) Layout() {
	...
	}
	{
		...
		lbl := eui.NewLabel(eui.GetLocale().Get("lblGmNr"), []int{xL, yL, w, h}, bg, fg)
		defer lbl.Close()
		lbl.SetBg(bg)
		lbl.Draw(r.Image)
	}
	...
	}
	{
		...
		lbl := eui.NewLabel(eui.GetLocale().Get("lblDTl"), []int{xL, yL, w, h}, bg, fg)
		defer lbl.Close()
		lbl.SetBg(bg)
		lbl.Draw(r.Image)
	}
	...
	{ // parse data green line, moves line
		...
		for i, j := 0, 1; j < len(results1)-2; i, j = i+2, j+2 { // level line
			...
			ebitenutil.DrawLine(r.Image, x1, y1, x2, y2, eui.GetTheme().Get("correct color"))
		}
	...
	{ // blue line and circle
		...
		for i, j := 0, 1; j < len(results1); i, j = i+2, j+2 {
			...
			boxSize := gridWidth / 2
			lbl := eui.NewLabel(strconv.Itoa(perc[k]), []int{int(x1) - boxSize/2, int(y1) - boxSize/2, boxSize, boxSize}, clrs[k], fg)
			defer lbl.Close()
			eui.DrawCircle(r.Image, x1, y1, float64(boxSize), clrs[k], true)
			lbl.Draw(r.Image)
			k++
		}
	}
